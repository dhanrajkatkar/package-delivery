...
class Bin:
    def __init__(self, size, length, width, height, capacity):
        ...
        self.total_items = 0  # number of total items in one container_bin
        self.items = []  # item in one container_bin -- a blank list initially
        self.unplaced_items = []
        self.unfitted_items = []  # unfitted item in one container_bin -- a blank list initially
    ...
    def can_hold_item_with_rotation(self, item, pivot):
        """Evaluate whether one item can be placed into container_bin with all optional orientations.
        Args:
        ...
        for i in range(0, len(RotationType.ALL)):
            ...
            if (
                ...
                if fit:
                    if self.get_total_weight() + item.weight > self.capacity:  # estimate whether container_bin reaches its capacity
                        fit = False
                        item.position = [0, 0, 0]
                        continue
    ...
    def put_item(self, item, pivot, distance_3d):
        """Evaluate whether an item can be placed into a certain container_bin with which orientation. If yes, perform put action.
        Args:
            item: any item in item list.
            pivot: an (x, y, z) coordinate, the back-lower-left corner of the item will be placed at the pivot.
            distance_3d: a 3D parameter to determine which orientation should be chosen.
        Returns:
            Boolean variable: False when an item couldn't be placed into the container_bin;
                              True when an item could be placed and perform put action.
        """
